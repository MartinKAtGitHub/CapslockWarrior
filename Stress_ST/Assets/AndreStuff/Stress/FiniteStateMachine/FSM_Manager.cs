...
public class FSM_Manager {
	...
	public void TheUpdate() {//denne metoden er den jeg forandret mest på. kanskje ikke til det bedre men følte det passet bedre til det jeg prøvde å få til 
		...
		if (_CurrentRunningState.Id != _StateToRun.Id) {//denne skjekker om den staten som skal testen er lik den staten som kjøres, hvis ikke så blir kjørende state satt til staten som skulle testes
			...
			_StateId = _CurrentRunningState.EnterState ();
			if (_StateId != "") {//an double check to see if im going to continue the current state
		...
		if (_StateId != "") {//hvis currentRunningState.ProcessState(); ikke returnerte null skal den neste statens som skal testes bli statId
			TheNextState (_StateId);
		}
	}

	//TODO make an override witch switches the states to what i want
}