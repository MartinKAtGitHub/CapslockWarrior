...
public class PropertyHolderEditor : Editor {
	...
	public override void OnInspectorGUI() {
		...
		case PropertyHolder.Status.B:            
			EditorGUILayout.PropertyField (controllable_Prop, new GUIContent ("controllable"));    
			//EditorGUILayout.IntField ("valForAB", valForAB_Prop);

			//	EditorGUILayout.IntField(valForAB_Prop, new GUIContent("valForAB") );
			EditorGUILayout.LabelField ("TestInt: ");
			((PropertyHolder)target).valForAB = EditorGUILayout.IntField (((PropertyHolder)target).valForAB);
			//EditorGUILayout.IntSlider ( valForAB_Prop, 0, 100, new GUIContent("valForAB") );
			break;
